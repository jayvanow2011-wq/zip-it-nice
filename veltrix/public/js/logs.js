...
export function bindLogs(view) {
    const list = view.querySelector("#log-list");
    async function load() {
        try {
            const logs = await api.logs();
            if (logs.length === 0) { list.innerHTML = '<div class="hc-muted" style="text-align:center;padding:20px">No logs</div>'; return; }
        ...
        } catch { list.innerHTML = '<div class="hc-muted" style="text-align:center;padding:20px">Failed to load logs</div>'; }
    }
    view.querySelector("#clear-logs").addEventListener("click", async () => { await api.clearLogs(); load(); });
    load();
}
