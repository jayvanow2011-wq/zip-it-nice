...
export function renderSettings(): string {
  return `
    <h1>Settings</h1>
    <p class="hc-sub">Configure notifications and integrations.</p>
    ...
    <div class="hc-card" style="max-width:520px;margin-top:12px">
      ...
      <div class="hc-info-row"><span>⚠️ Client went offline</span><span class="hc-muted">Status change alert</span></div>
      <div class="hc-info-row"><span>🔐 Login attempt</span><span class="hc-muted">Security alert</span></div>
    </div>`;
}

export function bindSettings(view: HTMLElement): void {
  const webhookInput = view.querySelector<HTMLInputElement>("#s-webhook")!;
  const saveBtn = view.querySelector<HTMLButtonElement>("#s-save")!;
  const testBtn = view.querySelector<HTMLButtonElement>("#s-test")!;
  const msgEl = view.querySelector<HTMLElement>("#s-msg")!;
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    msgEl.textContent = "";
    try {
      await api.saveSettings({ webhookUrl: webhookInput.value });
      msgEl.textContent = "Settings saved.";
    } catch (e) {
      msgEl.textContent = e instanceof Error ? e.message : "Failed";
    }
  ...
  testBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      msgEl.textContent = "Webhook sent! Check your Discord.";
    } catch (e) {
      msgEl.textContent = e instanceof Error ? e.message : "Webhook failed";
    }
