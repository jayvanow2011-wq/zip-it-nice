...
type Mode = "login" | "signup" | "authkey";
...
export function renderLogin(root: HTMLElement, onSuccess: () => void): void {
  ...
  function render(): void {
        ...
        <div class="hc-login-wrap">
          <div class="hc-login-card">
            ...
            <div style="margin:20px 0 12px;padding:16px;background:rgba(0,212,170,0.08);border:1px solid rgba(0,212,170,0.2);border-radius:8px">
              <div style="font-size:13px;color:#00d4aa;font-weight:600;margin-bottom:8px">✓ Account Created!</div>
              <div style="font-size:12px;color:#888;margin-bottom:12px">Save this auth key — you can use it to recover your account if you forget your password.</div>
              <div id="auth-key-display" style="font-family:'SF Mono',Menlo,monospace;font-size:13px;background:#000;border:1px solid #1a1a1a;border-radius:6px;padding:10px 12px;word-break:break-all;color:#ededed;cursor:pointer;user-select:all">${signupKey}</div>
              <button class="hc-connect-btn" id="copy-key-btn" style="margin-top:8px;font-size:11px">📋 Copy Key</button>
            </div>
            <p style="font-size:11px;color:#ff4444;margin:8px 0">⚠ This key will NOT be shown again. Save it now!</p>
            <button class="hc-primary" id="continue-login-btn">Continue to Login</button>
          </div>
        </div>`;
      root.querySelector("#copy-key-btn")!.addEventListener("click", () => {
        navigator.clipboard.writeText(signupKey!);
      });
      root.querySelector("#continue-login-btn")!.addEventListener("click", () => {
        signupKey = null;
        mode = "login";
        render();
      });
    ...
    const tabs = [
      ...
      { id: "authkey" as Mode, label: "Auth Key" },
    ];
    ...
    if (mode === "login") {
      ...
      formHtml = `
        <form id="signup-form">
          <p class="hc-sub">Create a new account. You'll receive an auth key for recovery.</p>
          <label>Username</label>
    ...
    // Bind tab clicks
    root.querySelectorAll<HTMLButtonElement>(".hc-login-tab").forEach(btn => {
    ...
    if (mode === "login") {
      root.querySelector<HTMLFormElement>("#login-form")!.addEventListener("submit", async (e) => {
        e.preventDefault();
        const user = (root.querySelector("#f-user") as HTMLInputElement).value;
        const pass = (root.querySelector("#f-pass") as HTMLInputElement).value;
      ...
      root.querySelector<HTMLFormElement>("#signup-form")!.addEventListener("submit", async (e) => {
        e.preventDefault();
        const user = (root.querySelector("#f-user") as HTMLInputElement).value;
        const pass = (root.querySelector("#f-pass") as HTMLInputElement).value;
        const pass2 = (root.querySelector("#f-pass2") as HTMLInputElement).value;
        if (pass !== pass2) {
          errEl.textContent = "Passwords do not match";
          errEl.style.display = "block";
          return;
        }
        const btn = root.querySelector<HTMLButtonElement>("#signup-btn")!;
        btn.disabled = true;
        btn.textContent = "Creating account…";
        errEl.style.display = "none";
        try {
          const res = await api.signup(user, pass);
          signupKey = res.authKey;
          render();
        } catch (err) {
          errEl.textContent = err instanceof Error ? err.message : "Signup failed";
          errEl.style.display = "block";
          btn.disabled = false;
      ...
      root.querySelector<HTMLFormElement>("#authkey-form")!.addEventListener("submit", async (e) => {
        e.preventDefault();
        const key = (root.querySelector("#f-authkey") as HTMLInputElement).value;
        const btn = root.querySelector<HTMLButtonElement>("#authkey-btn")!;
        btn.disabled = true;
        btn.textContent = "Verifying…";
        errEl.style.display = "none";
        try {
          await api.loginAuthKey(key);
          onSuccess();
        } catch (err) {
          errEl.textContent = err instanceof Error ? err.message : "Invalid auth key";
          errEl.style.display = "block";
          btn.disabled = false;
          btn.textContent = "Login with Auth Key";
        }
      });
    }
  }
