...
export function renderDashboard(data: StatsResponse): string {
  ...
  const cards = [
    { label: "TOTAL CLIENTS", value: a.total, icon: "👥", grad: "hc-grad-1" },
    { label: "ACTIVE NOW", value: a.online, icon: "📡", grad: "hc-grad-2" },
    { label: "OFFLINE", value: a.offline, icon: "📉", grad: "hc-grad-3" },
    { label: "METRICS 24H", value: a.builds || 0, icon: "📊", grad: "hc-grad-4" },
  ];
  ...
  return `
    ...
    <div class="hc-overview-grid">
      ${cards.map(c => `
        <div class="hc-overview-card ${c.grad}">
          <div class="hc-overview-card-top">
            <span class="hc-overview-label">${c.label}</span>
            <span class="hc-overview-icon">${c.icon}</span>
          </div>
    ...
    <div class="hc-card hc-chart-card">
      <div style="display:flex;justify-content:flex-end;gap:16px;margin-bottom:12px">
        <span style="display:flex;align-items:center;gap:4px;font-size:12px">
          <span style="width:8px;height:8px;border-radius:50%;background:#00d4aa;display:inline-block"></span> CPU
        </span>
        <span style="display:flex;align-items:center;gap:4px;font-size:12px">
          <span style="width:8px;height:8px;border-radius:50%;background:#6366f1;display:inline-block"></span> RAM
        </span>
      </div>
